...
- `experimental_features` — comma-separated independent flags, with no spaces: `sched_tuning`, `audio_jack_ir`, `bt_hci_snoop`, `bt_audio_transport`, `wowlan_keepalive`. Leave blank to disable all. For example, `sched_tuning,bt_hci_snoop` enables those two only. This shared input keeps the workflow within GitHub's 25-input limit.
